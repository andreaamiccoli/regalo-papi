(function () {
  ...
  /* ---------------------------------------------------------
     STATO PER LA FUNZIONALITÀ "SCUOTI PER I BIGLIETTI"
  --------------------------------------------------------- */
  const flipCard = document.getElementById("flip-card");
  const shakeHint = document.getElementById("shake-hint");
  const backHint = document.getElementById("back-hint");

  const SHAKE_THRESHOLD = 16;      // soglia di accelerazione (m/s^2) per rilevare lo scuotimento
  const SHAKE_COOLDOWN_MS = 1200;  // evita che uno scuotimento venga letto più volte di seguito
  ...
  function revealAll() {
    ...
    // Da qui in poi si può scuotere il telefono per vedere i biglietti
    if (shakeHint) {
  ...
  /* ---------------------------------------------------------
     RILEVAMENTO DELLO SCUOTIMENTO (DeviceMotion)
     Su iOS 13+ l'accesso ai sensori di movimento richiede un
     permesso esplicito, che va richiesto in risposta diretta
     a un gesto dell'utente (qui: il tocco che completa il gratta).
  --------------------------------------------------------- */
  function enableShakeDetection() {
    if (typeof DeviceMotionEvent === "undefined") {
      return; // dispositivo/browser senza supporto: restano solo i pulsanti di fallback
    }
    ...
    if (!hasIOSPermissionApi) {
      // Android e la maggior parte dei browser desktop: nessun permesso da chiedere
    ...
    DeviceMotionEvent.requestPermission()
      ...
      .catch((error) => {
        console.error("Permesso sensori di movimento negato/fallito:", error);
      });
  }
  ...
  function handleDeviceMotion(event) {
    ...
    if (shakeForce > SHAKE_THRESHOLD && now - lastShakeTime > SHAKE_COOLDOWN_MS) {
      lastShakeTime = now;
      toggleFlip();
    }
  }
